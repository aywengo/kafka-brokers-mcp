...
def create_topics(admin_client: AdminClient, cluster_name: str) -> List[str]:
    ...
    topics_to_create = [
        NewTopic("user-events", num_partitions=6, replication_factor=1, config={
            'retention.ms': '604800000',  # 7 days
            'cleanup.policy': 'delete'
        }),
        NewTopic("order-updates", num_partitions=3, replication_factor=1, config={
            'retention.ms': '86400000',   # 1 day
            'cleanup.policy': 'delete'
        }),
        NewTopic("payment-notifications", num_partitions=2, replication_factor=1, config={
            'retention.ms': '2592000000', # 30 days
            'cleanup.policy': 'delete'
        }),
        NewTopic("analytics-events", num_partitions=4, replication_factor=1, config={
            'retention.ms': '2592000000', # 30 days
            'cleanup.policy': 'delete'
        }),
        NewTopic("system-logs", num_partitions=8, replication_factor=1, config={
            'retention.ms': '604800000',  # 7 days
            'cleanup.policy': 'delete'
        })
    ]
...
def create_consumer_groups(consumer_config: Dict[str, Any], topics: List[str]):
    ...
    consumer_groups = [
        ...
        {
            'group_id': 'order-fulfillment',
            'topics': ['order-updates'],
            'description': 'Handles order fulfillment workflow'
        },
        {
        ...
        },
        {
            ...
            'topics': ['system-logs'],
            'description': 'Monitors system logs for issues'
        },
        {
